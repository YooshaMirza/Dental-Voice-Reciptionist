...
Ported from voice_agent/call_prompts.py unchanged, except:
- Django settings/DB imports replaced with the FastAPI equivalents.
- `replace_placeholders` now also does a generic pass over any remaining
  `{{key}}`/`{?key?}` tokens using whatever extra fields are present in
  `data` (e.g. arbitrary fields from a GHL lead payload), so new GHL fields
  can be referenced in the prompt text without another code change.
"""
import re
import logging
import datetime
...
from app.db.mongo import prompts_collection
...
def replace_placeholders(prompt: str, data: dict) -> str:
    ...
    replacements = {
        ...
        # Generic lead-context field, populated once real GHL payload fields are known
        # (e.g. "missing_teeth"). Falls back to empty string so unknown placeholders
        # never leak into the spoken prompt.
        "lead_context_summary": data.get("lead_context_summary") or "",
    }
    ...
    # Generic pass: any {{some_field}} left over gets filled from `data` directly if present,
    # so arbitrary GHL lead fields work without further code changes once their real names
    # are known. Anything still unresolved is left as-is rather than guessed at.
    def _fill_remaining(match: "re.Match") -> str:
        key = match.group(1)
        if key in data and data.get(key) not in (None, ""):
            return str(data[key])
        return match.group(0)

    formatted_prompt = re.sub(r"\{\{(\w+)\}\}", _fill_remaining, formatted_prompt)
    formatted_prompt = re.sub(r"\{\?(\w+)\?\}", _fill_remaining, formatted_prompt)
...
*   **If the "Before the call" intake info above already tells you something specific** (e.g. how long they've been missing teeth, how many, or their condition): open with a warm acknowledgment stating that specific fact back to them, then ask EXACTLY: "Can you tell me more about it?" Do NOT re-ask a clarifying question you already have the answer to from the intake form.
    *   Example: "I see here you've been missing your teeth for more than a year now. Can you tell me more about it?"
*   **If you have no intake info to go on**, acknowledge their dental situation with empathy, then ask EXACTLY two of the following questions (ask one at a time):
...
def get_system_prompt(customer_data: dict, call_type: str = "onboarding", current_time: str | None = None) -> str:
    ...
    try:
        ...
        if db_prompt and db_prompt.get("prompt_text"):
            ...
            raw_prompt = OUTBOUND_SYSTEM_PROMPT if direction == "outbound" else INBOUND_SYSTEM_PROMPT
            prompts_collection.update_one(
                {"prompt_type": direction},
                {
                    "$set": {
                        "prompt_type": direction,
                        "title": "Outbound Patient Advocate (Dentina)" if direction == "outbound" else "Inbound Assistant (Alice)",
    ...
    return replace_placeholders(raw_prompt, customer_data)
