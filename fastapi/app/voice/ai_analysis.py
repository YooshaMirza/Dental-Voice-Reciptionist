"""
AI Analysis Module — transcription + sentiment/outcome analysis for call
recordings. Ported from voice_agent/ai_analysis.py unchanged apart from
import paths.
"""
import asyncio
import logging
...
from app.db.calls import (
    update_call_transcript,
    update_call_recording,
    update_call_ai_status,
    get_call_by_sid,
    save_feedback,
)
...
async def process_recording_with_fetch(call_sid: str, call_end_time, retry_count: int = 0, max_retries: int = 2) -> bool:
    try:
        call_data = get_call_by_sid(call_sid)
        ...
        if not recording_url:
            logger.warning(f"[call_{call_sid[:8]}] No recording URL found in DB (attempt {retry_count + 1}/{max_retries})")
            if call_data and call_data.get("transcript") and call_data["transcript"].get("text") and not call_data.get("ai_analysis"):
                ...
                return await process_transcript_analysis_only(
                    call_sid, call_data["transcript"]["text"], call_data.get("customer_number") or call_data.get("phone_number")
                )
            return False
...
async def process_transcript_analysis_only(call_sid: str, transcript_text: str, phone_number: str) -> bool:
    ...
    try:
        await asyncio.to_thread(update_call_ai_status, call_sid, "processing")
        ...
        if turns:
            await asyncio.to_thread(update_call_transcript, call_sid, transcript_text, turns)
...
async def process_recording_analysis(call_sid: str, recording_url: str) -> bool:
    """Download, transcribe, and analyze a call recording."""
    try:
        logger.info(f"[AI ANALYSIS] [START] Initiating analysis pipeline for Call SID: {call_sid}")
        logger.info("[AI ANALYSIS] [Step 1/4] Downloading audio recording from Twilio...")
        audio_content = None
        ...
        calls_collection.update_one(
            {"call_sid": call_sid},
            {"$set": {"recording_file_data": audio_content, "recording_file_saved_at": datetime.utcnow()}},
        )
        logger.info(f"Saved binary audio recording directly to MongoDB for {call_sid}")

        call_data = await asyncio.to_thread(get_call_by_sid, call_sid)
        duration = call_data.get("call_duration", 0) if call_data else 0

        if duration and 0 < duration <= 3:
            logger.info(f"[call_{call_sid[:8]}] Skipping short call ({duration}s)")
            await asyncio.to_thread(update_call_ai_status, call_sid, "skipped_short_call")
            await asyncio.to_thread(update_call_recording, call_sid, recording_url, duration)
            return True

        logger.info("[AI ANALYSIS] [Step 3/4] Transcribing audio content via Gemini...")
        await asyncio.to_thread(update_call_ai_status, call_sid, "processing")
        ...
        await asyncio.to_thread(update_call_transcript, call_sid, transcript_data["full_text"], transcript_data["turns"])

        logger.info("[AI ANALYSIS] [Step 4/4] Analyzing transcript text (sentiment, outcomes, details)...")
        await _analyze_with_ai(call_sid, transcript_data["full_text"], call_data.get("phone_number") if call_data else None)
...
async def _transcribe_with_gemini(call_sid: str, audio_content: bytes) -> dict:
    try:
        ...
        genai_sdk.configure(api_key=load_config().get("GEMINI_API_KEY", ""))
...
"""
        ...
        await asyncio.to_thread(update_call_ai_status, call_sid, "failed")
